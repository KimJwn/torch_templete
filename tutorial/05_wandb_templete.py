    ...
    def __init__(
        ...
        if self.gpu_id == 0:    
            ...
            if pg_name!= False:
                import wandb
                wandb.login
                wandb.init(project=pj_name, entity="merily", name=pg_name, id=pg_name, resume="allow")
...
'''*Newrly Updated For Torchrun Multi GPU DDP*'''
def main(save_every: int, total_epochs: int, batch_size: int, pj_name:str, wandb_pg_name, snapshot_path: str = "snapshot.pt"):
    ddp_setup()
    ...
    trainer = Trainer(model, train_loader, optimizer, F.cross_entropy, save_every, snapshot_path, pj_name, wandb_pg_name, valid_loader)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--wandb_pg_name', '-w', default=False, help='Would you connect to Wandb Server')
    args = parser.parse_args()
    
    main(args.save_every, args.total_epochs, args.batch_size, args.pj_name, args.wandb_pg_name)